...
def developable(model: nn.Module, config: TrainingConfig, surface_points: Tensor, t: float) -> dict[str, Tensor]:
    ...
    surface_for_near = surface_points[
        torch.randperm(surface_points.shape[0], device=surface_points.device)[:n_near]
    ]
    ...
    loss_developable = loss_weights.developable(t) * double_trace_loss(
        surface_and_near, grad_for_developable
    )
